...
/*   Updated: 2018/06/13 12:14:18 by jjauzion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ldi(t_process *process, t_arena *arena)
{
	...
	arg[0][0] = get_arg_val(process, 0, arena->mem, process->pc + 2);
	arg_size = (process->op->arg_type[0] == T_REG) ? 1 : 2;
	arg[1][0] = get_arg_val(process, 1, arena->mem, process->pc + 2 + arg_size);
	address = (arg[1][0] + arg[0][0]) % IDX_MOD;
	arg_size += (process->op->arg_type[1] == T_REG) ? 1 : 2;
	arg[2][0] = get_arg_id(process, 2, arena->mem, process->pc + 2 + arg_size);
	value = mem2int(arena->mem, process->pc + (int)address, REG_SIZE);
	int2reg(process, (t_uint)arg[2][0], value);
	arg[0][1] = -1;
	arg[1][1] = -1;
	arg[2][1] = arg[2][0];
	show_operation(arena, process, arg, "load from");
	if (value == 0)
		process->carry = 1;
	else
		process->carry = 0;
