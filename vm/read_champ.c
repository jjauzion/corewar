...
/*   Updated: 2018/06/19 16:44:52 by jjauzion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_champion		*read_champ(char *file)
{
	int			j;
	int			fd;
	t_uchar		buff[2 * (int)S_UINT + CHAMP_MAX_SIZE + PROG_NAME_LENGTH + 1 + COMMENT_LENGTH + 1 + 6];
	t_champion	*champion;
	void		*ptr;
	int			nb_byte;
	int			ret;
	...
	if (ret < (2 * nb_byte + CHAMP_MAX_SIZE + PROG_NAME_LENGTH + COMMENT_LENGTH + 8))
