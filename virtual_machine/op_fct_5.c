...
/*   Updated: 2018/06/20 15:42:36 by jjauzion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		fork_op(t_process *process, t_arena *arena)
{
	...
	if (!(new_process = create_process(
					get_address(process->pc + (arg % IDX_MOD)), reg1, process)))
		return (ERROR);
	new_process->op = NULL;
	arena->nb_process++;
...
int		lfork_op(t_process *process, t_arena *arena)
{
	...
	reg1 = reg2int(process, 1);
	if (!(new_process = create_process(
					get_address(process->pc + arg), reg1, process)))
		return (ERROR);
	new_process->op = NULL;
	arena->nb_process++;
	new_process->next = arena->process;
